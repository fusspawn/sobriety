...
namespace Utils.Procedural
{
    ...
    public class PerlinNoise
    {
        ...
        public static Byte[] GetPerlinArray(int X, int Y, int Octaves = 1) {
            PerlinNoise NoiseGenerator = new PerlinNoise(new Random().Next());
            Byte[] NoiseData = new Byte[X * Y];
            for (int x = 0; x < X; x++) {
                for (int y = 0; y < Y; y++) {
                    for(int oct = 0; oct < Octaves; oct++)
                        NoiseData[X * x + y] = (byte)((NoiseGenerator.Noise(2 * x, 2 * y, -0.5)) * 255);
                }
            }
